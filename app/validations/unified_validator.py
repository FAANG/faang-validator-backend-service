    ...
    def export_valid_samples_to_biosample(self, validation_results: Dict[str, Any]) -> Dict[str, List[Dict]]:
        ...
        for sample_type in sample_types:
            ...
            if valid_samples_key in results:
                print(f"  Count in {valid_samples_key}: {len(results[valid_samples_key]) if results[valid_samples_key] else 0}")
                    ...
                    try:
                        ...
                        if normalized_sample_type in ['specimen_from_organism', 'pool_of_specimens']:
                            ...
                            if 'organism' not in characteristics or 'species' not in characteristics:
                                ...
                                print(f"  Sample {sample_name_export}: parent_name={parent_name}, sample_type={normalized_sample_type}")
                                    ...
                                    if normalized_sample_type == 'specimen_from_organism':
                                        ...
                                        if parent_name in organism_samples:
                                            ...
                                            if hasattr(parent_model, 'organism') and hasattr(parent_model, 'organism_term_source_id'):
                                                ...
                                                parent_biosample_data = org_validator.export_to_biosample_format(parent_model)
                                        ...
                                        specimen_results = results_by_type.get('specimen from organism', {}) or results_by_type.get('specimen_from_organism', {}) or {}
                                        specimen_valid_key = 'valid_specimen_from_organisms'
                                        print(f"  Pool sample {sample_name_export}: Looking for parent specimen '{parent_name}' in {specimen_valid_key}")
                                        print(f"  Available specimen samples: {[s.get('sample_name') for s in specimen_results.get(specimen_valid_key, [])][:5]}")
                                                ...
                                                if spec_sample.get('sample_name') == parent_name:
                                                    ...
                                                    if spec_parent_name and spec_parent_name in organism_samples:
                                                        ...
                                                        if hasattr(parent_model, 'organism') and hasattr(parent_model, 'organism_term_source_id'):
                                                            ...
                                                            org_validator = OrganismValidator()
                                                            parent_biosample_data = org_validator.export_to_biosample_format(parent_model)
                                                            print(f"  Got organism data from parent: {parent_biosample_data.get('characteristics', {}).get('organism')}")
                                                    else:
                                                        print(f"  Parent organism '{spec_parent_name}' not found in organism_samples")
                                                    break
                                            if not found_specimen:
                                                print(f"  Parent specimen '{parent_name}' not found in specimen_results")
                        ...
                        print(f"  [{idx + 1}/{len(results[valid_samples_key])}] Failed to export {sample_name_export}: {str(e)}")
                        import traceback
                        traceback.print_exc()
